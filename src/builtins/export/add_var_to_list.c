...
/*   Updated: 2024/07/29 11:46:22 by derjavec         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	add_env(t_mini *mini, char *new_var)
{
	...
	if (split_new_envp[0][ft_strlen(split_new_envp[0]) - 1] == '+')
	{
		...
		if (!concat)
			return (free_tab(split_new_envp), ft_error(mini, NULL, strerror(errno)), -1);
...
static int	check_syn_error(t_mini *mini, char	*new_var)
{
	...
	{
		mini->exit_status = 1;
		return (ft_error(mini, " not a valid identifier1", NULL), -1);
	}
	i = 1;
	while (new_var[i] && new_var[i] != '=')
	{
		if (ft_isalnum(new_var[i]) == 0 && new_var[i] != '_' \
			&& (new_var[i] != '+' && new_var[i + 1] && new_var[i + 1] != '='))
		{
			mini->exit_status = 1;
			return (ft_error(mini, " not a valid identifier2", NULL), -1);
		}
		i++;
	}
	return (0);
