...
/*   Updated: 2024/06/20 11:23:10 by deniseerjav      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static int	add_with_nulll_value(t_mini *mini, char *new_var, char	***split_new_exp)
{
    char	*value;
    
    value = get_env_value(mini->export, new_var);
    if (!value)
        return (ft_error("Malloc in export function", mini), 1);
    if (value[0] != '\0')
        return (free(value), 1);
    free(value);
